...
*implementation. This kernel is designed to process image by blocks of blocks8x8
*that utilizes maximum warps capacity, assuming that it is enough of 8 threads
*  per block8x8.
*
...
__global__ void DCT2D8x8(float *dst, const float *src, const uint size) {
  ...
  if (blockIdx.x * KER2_BLOCK_HEIGHT * KER2_BLOCK_WIDTH +
          (threadIdx.y + 1) * BLOCK_SIZE * BLOCK_SIZE - 1 >=
      size)
    return;
  ...
  // 8x1 blocks in one macro-block (threadIdx.y - index of block inside the
  // macro-block) Get the first element of the column in the block with index
  // threadIdx.y
  src += offset;
  dst += offset;
...
#pragma unroll
  ...
  for (unsigned int i = 0; i < BLOCK_SIZE; i++)
    bl_ptr[i * BLOCK_SIZE] =
        src[i * BLOCK_SIZE];  // Load column to the shared mem
...
extern "C" void run_DCT2D8x8(float *transformed_stacks,
                             const float *gathered_stacks, const uint size,
                             const dim3 num_threads, const dim3 num_blocks) {
  DCT2D8x8<<<num_blocks, num_threads>>>(transformed_stacks, gathered_stacks,
                                        size);
}

extern "C" void run_IDCT2D8x8(float *gathered_stacks,
                              const float *transformed_stacks, const uint size,
                              const dim3 num_threads, const dim3 num_blocks) {
  IDCT2D8x8<<<num_blocks, num_threads>>>(gathered_stacks, transformed_stacks,
                                         size);
}
