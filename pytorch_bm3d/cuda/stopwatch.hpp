...
/**
 * \brief Implementation of high precision wall-time stopwatch based on system
 * timers.
 */
class Stopwatch {
 private:
  ...
  ticks_t now() {
    ...
    return static_cast<ticks_t>(ts.tv_sec) * 1000000000UL +
           static_cast<ticks_t>(ts.tv_nsec);
#endif
  }
  ...
  void measureTime() {
    ...
    mInterval += static_cast<double>(now() - mStartTime) /
 ...
 public:
  ...
  /**
   * \brief Start the stopwatch. If the stopwatch are already timing, they are
   * reset.
   */
  void start() {
    mTiming = true;
