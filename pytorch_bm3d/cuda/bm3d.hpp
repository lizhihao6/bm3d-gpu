...
extern "C" void run_block_matching(
    const raw_int* __restrict image, ushort* stacks, uint* num_patches_in_stack,
...
extern "C" void run_get_block(const uint2 start_point,
                              const raw_int* __restrict image,
                              const ushort* __restrict stacks,
                              const uint* __restrict num_patches_in_stack,
                              float* patch_stack, const uint2 image_dim,
                              const uint2 stacks_dim, const Params params,
                              const dim3 num_threads, const dim3 num_blocks);

extern "C" void run_DCT2D8x8(float* d_transformed_stacks,
                             const float* d_gathered_stacks, const uint size,
                             const dim3 num_threads, const dim3 num_blocks);
...
extern "C" void run_IDCT2D8x8(float* d_gathered_stacks,
                              ...
                              const uint size, const dim3 num_threads,
                              const dim3 num_blocks);

extern "C" void run_aggregate_block(
    const uint2 start_point, const float* __restrict patch_stack,
    const float* __restrict w_P, const ushort* __restrict stacks,
    const float* __restrict kaiser_window, float* numerator, float* denominator,
    const uint* __restrict num_patches_in_stack, const uint2 image_dim,
    const uint2 stacks_dim, const Params params, const dim3 num_threads,
    const dim3 num_blocks);

extern "C" void run_aggregate_final(const float* __restrict numerator,
                                    const float* __restrict denominator,
                                    const uint2 image_dim,
                                    raw_int* denoised_noisy_image,
...
#define cuda_error_check(ans) \
  { throw_on_cuda_error((ans), __FILE__, __LINE__); }
...
class BM3D {
 private:
  ...
  std::vector<raw_int*> d_noisy_image;
  std::vector<raw_int*> d_denoised_image;

  // Auxiliary arrays
  ushort* d_stacks;  // Addresses of similar patches to each reference patch of
                     // a batch
  ...
  uint* d_num_patches_in_stack;  // Number of similar patches for each referenca
                                 // patch of a batch that are stored in d_stacks
  float* d_gathered_stacks;      // 3D groups of a batch
  float* d_gathered_stacks_basic;  // Only for two step denoising, contains
                                   // wiener coefficients
  ...
  uint2 h_batch_size;  // h_batch_size.x has to be divisible by
                       // properties.warpSize
  ...
  void allocate_device_auxiliary_arrays() {
    ...
    cuda_error_check(
        cudaMalloc((void**)&d_stacks,
                   sizeof(ushort) * h_batch_size.x * h_batch_size.y * maxN));

    cuda_error_check(
        cudaMalloc((void**)&d_num_patches_in_stack,
                   sizeof(uint) * h_batch_size.x * h_batch_size.y));

    cuda_error_check(cudaMalloc((void**)&d_gathered_stacks,
                                sizeof(float) * (maxN + 1) * maxk * maxk *
                                    h_batch_size.x * h_batch_size.y));

    cuda_error_check(cudaMalloc(
        (void**)&d_w_P, sizeof(float) * h_batch_size.x * h_batch_size.y));

    cuda_error_check(
        cudaMalloc((void**)&d_kaiser_window, sizeof(float) * maxk * maxk));

    if (h_reserved_two_step)
      cuda_error_check(cudaMalloc((void**)&d_gathered_stacks_basic,
                                  sizeof(float) * (maxN + 1) * maxk * maxk *
                                      h_batch_size.x * h_batch_size.y));
  }
  ...
  void allocate_device_image(uint width, uint height, uint channels) {
    ...
    for (auto& it : d_noisy_image) {
      cuda_error_check(cudaMalloc((void**)&it, sizeof(raw_int) * size));
    }
  ...
  // Creates an kaiser window (only for k = 8, alpha = 2.0) and copies it to the
  // device.
  void prepare_kaiser_window(uint k) {
    ...
    if (k == 8) {
      ...
      for (unsigned i = 0; i < k * k; i++) kaiserWindow[i] = 1.0f;
  ...
  void copy_device_image(const raw_int* src_image, int width, int height,
                         int channels) {
    size_t image_size = width * height;
    for (int i = 0; i < channels; ++i) {
      // Copy image to device
      cuda_error_check(cudaMemcpy(d_noisy_image[i], src_image + i * image_size,
                                  image_size * sizeof(raw_int),
  ...
  {
    // Determine number of warps form block-matching according to the size of
    // shared memory.
    const uint p_block_width =
        ((properties.warpSize - 1) * params.p) + params.k;
    const uint s_image_p_size = p_block_width * params.k * sizeof(raw_int);

    const float shared_mem_usage = 1.0f;  // 0 - 1
    const uint shared_mem_avaliable =
        (uint)(properties.sharedMemPerBlock * shared_mem_usage) -
        s_image_p_size;

    // Block-matching shared memory sizes per warp
    const uint s_diff_size = p_block_width * sizeof(uint);
    const uint s_patches_in_stack_size = properties.warpSize * sizeof(raw_int);
    const uint s_patch_stacks_size =
        params.N * properties.warpSize * sizeof(uint);

    const uint num_warps =
        std::min(shared_mem_avaliable / (s_diff_size + s_patches_in_stack_size +
                                         s_patch_stacks_size),
                 32u);

    // Block-matching Launch parameters
    s_mem_size =
        ((s_diff_size + s_patches_in_stack_size + s_patch_stacks_size) *
         num_warps) +
        s_image_p_size;
    ...
    if (_verbose) {
      std::cout << "Shared memory : " << s_mem_size / 1024 << "KB/"
                << properties.sharedMemPerBlock / 1024 << "KB" << std::endl;
      std::cout << "Number of warps: " << num_warps << std::endl;
    }
  }

  /*
  Launch first step of BM3D. It produces basic estimate in denoised_image
  arrays.
  */
  void first_step(std::vector<raw_int*>& denoised_image, int width, int height,
    ...
    const uint2 stacks_dim = make_uint2(width - (h_hard_params.k - 1),
                                        height - (h_hard_params.k - 1));

    int paramN1 =
        h_hard_params.N + 1;  // maximal size of a stack with a reference patch

    // Determine launch parameteres for block-matching kernel
    dim3 num_threads_bm;
    dim3 num_blocks_bm;
    uint s_size_bm;
    get_BM_launch_parameters(h_hard_params, num_threads_bm, num_blocks_bm,
                             s_size_bm);
    ...
    const uint trans_size = h_hard_params.k * h_hard_params.k * paramN1 *
                            h_batch_size.x * h_batch_size.y;
    const dim3 num_blocks_tr(
        (trans_size + (KER2_BLOCK_WIDTH * h_hard_params.k) - 1) /
            (KER2_BLOCK_WIDTH * h_hard_params.k),
        1, 1);
    const dim3 num_threads_tr(h_hard_params.k,
                              KER2_BLOCK_WIDTH / h_hard_params.k, 1);

    // Determine launch parameteres for filtering kernel
    const uint s_size_t = h_hard_params.k * h_hard_params.k * (paramN1 + 1) *
                          sizeof(float);  //+1 for avoinding bank conflicts

    // Determine launch parameteres for final division kernel
    const dim3 num_threads_f(128, 4);
    const dim3 num_blocks_f((width + num_threads_f.x - 1) / num_threads_f.x,
                            (height + num_threads_f.y - 1) / num_threads_f.y);
    ...
    // Batch processing: in each iteration only the batch_size reference patches
    // are processed.
    uint2 start_point;
    for (start_point.y = 0; start_point.y < stacks_dim.y + h_hard_params.p - 1;
         start_point.y += (h_batch_size.y * h_hard_params.p)) {
      // Show progress
      if (_verbose) {
        int percent =
            (int)(((float)start_point.y / (float)stacks_dim.y) * (float)100);
        std::cout << "\rProcessing " << percent << "%" << std::flush;
      }
      for (start_point.x = 0;
           start_point.x < stacks_dim.x + h_hard_params.p - 1;
           start_point.x += (h_batch_size.x * h_hard_params.p)) {
        if (_verbose) time_blockmatching.start();

        // Finds similar patches for each reference patch of a batch and stores
        // them in d_stacks array
        run_block_matching(
            d_noisy_image[0],  // IN: Image
            d_stacks,          // OUT: Array of adresses of similar patches
            d_num_patches_in_stack,  // OUT: Array containing numbers of these
                                     // addresses
            image_dim,               // IN: Image dimensions
            stacks_dim,      // IN: Dimensions limiting addresses of reference
                             // patches
            h_hard_params,   // IN: Denoising parameters
            start_point,     // IN: Address of the top-left reference patch of a
                             // batch
        ...
        if (_verbose) time_blockmatching.stop();

        for (int channel = 0; channel < channels; ++channel) {
          if (_verbose) time_get.start();
          ...
          run_DCT2D8x8(d_gathered_stacks, d_gathered_stacks, trans_size,
                       num_threads_tr, num_blocks_tr);
          cuda_error_check(cudaGetLastError());
          cuda_error_check(cudaDeviceSynchronize());
          ...
          /*
          1) 1D Walsh-Hadamard transform of proper size on the 3rd dimension of
          each 3D group of a batch to complete the 3D transform. 2) Hard
          thresholding 3) Inverse 1D Walsh-Hadamard trannsform. 4) Compute the
          weingt of each 3D group
          */
          ...
          run_IDCT2D8x8(d_gathered_stacks, d_gathered_stacks, trans_size,
                        num_threads_tr, num_blocks_tr);
          ...
          // Aggregates filtered patches of all 3D groups of a batch into
          // numerator and denominator buffers
          run_aggregate_block(
              start_point,        // IN: First reference patch of a batch
              d_gathered_stacks,  // IN: 3D groups with thransfomed patches
              d_w_P,            // IN: Numbers of non zero coeficients after 3D
                                // thresholding
              d_stacks,         // IN: Array of adresses of similar patches
          ...
          if (_verbose) time_aggregate.stop();
        }
      }
    }
    ...
    if (_verbose) {
      ...
      std::cout << "  Block-Matching took: " << time_blockmatching.getSeconds()
                << std::endl;
      std::cout << "  Get took: " << time_get.getSeconds() << std::endl;
      std::cout << "  Transform took: " << time_transform.getSeconds()
                << std::endl;
      std::cout << "  Tresholding took: " << time_treshold.getSeconds()
                << std::endl;
      std::cout << "  Inverse transform took: " << time_itransform.getSeconds()
                << std::endl;
      std::cout << "  Aggregation took: " << time_aggregate.getSeconds()
                << std::endl;
    }
  }
  ...
  void second_step(std::vector<raw_int*>& denoised_image, int width, int height,
    ...
    // Dimensions limiting addresses of reference patches
    const uint2 stacks_dim = make_uint2(width - (h_wien_params.k - 1),
                                        height - (h_wien_params.k - 1));

    int paramN1 =
        h_wien_params.N + 1;  // Maximal size of a stack with a reference patch

    // Determine launch parameteres for block-matching kernel
    dim3 num_threads_bm;
    dim3 num_blocks_bm;
    uint s_size_bm;
    get_BM_launch_parameters(h_wien_params, num_threads_bm, num_blocks_bm,
                             s_size_bm);
    ...
    // Determine launch parameteres for DCT kernel
    const uint trans_size = h_wien_params.k * h_wien_params.k * paramN1 *
                            h_batch_size.x * h_batch_size.y;
    const dim3 num_blocks_tr(
        (trans_size + (KER2_BLOCK_WIDTH * h_wien_params.k) - 1) /
            (KER2_BLOCK_WIDTH * h_wien_params.k),
        1, 1);
    const dim3 num_threads_tr(h_wien_params.k,
                              KER2_BLOCK_WIDTH / h_wien_params.k, 1);

    // Determine launch parameteres for filtering kernel
    const uint s_size_t = 2 * h_wien_params.k * h_wien_params.k *
                          (paramN1 + 1) *
                          sizeof(float);  //+1 for avoinding bank conflicts
    ...
    for (start_point.y = 0; start_point.y < stacks_dim.y + h_wien_params.p - 1;
         start_point.y += (h_batch_size.y * h_wien_params.p)) {
      // Show progress
      if (_verbose) {
        int percent =
            (int)(((float)start_point.y / (float)stacks_dim.y) * (float)100);
      ...
      for (start_point.x = 0;
           start_point.x < stacks_dim.x + h_wien_params.p - 1;
           start_point.x += (h_batch_size.x * h_wien_params.p)) {
        if (_verbose) time_blockmatching.start();
        ...
        for (int channel = 0; channel < channels; ++channel) {
          ...
          // Get patches from basic image estimate to 3D auxiliary array
          // according to the addresess form block-matching
          run_get_block(
              start_point,  // IN: First reference patch of a batch
              d_denoised_image[channel],  // IN: Basic image estimate (produced
                                          // by 1st step)
              d_stacks,  // IN: Array of adresses of similar patches
              d_num_patches_in_stack,   // IN: Numbers of patches in 3D groups
          ...
          // Get patches from noisy image to 3D auxiliary array according to the
          // addresess form block-matching
          run_get_block(
              start_point,             // IN: First reference patch of a batch
              d_noisy_image[channel],  // IN: Basic image estimate (produced by
                                       // 1st step)
              d_stacks,  // IN: Array of adresses of similar patches
              d_num_patches_in_stack,  // IN: Numbers of patches in 3D groups
              d_gathered_stacks,       // OUT: Assembled 3D groups
          ...
          // Apply 2D DCT transform to each layer of 3D group that contains
          // noisy patches
          run_DCT2D8x8(d_gathered_stacks, d_gathered_stacks, trans_size,
                       num_threads_tr, num_blocks_tr);
          cuda_error_check(cudaGetLastError());
          ...
          // Apply 2D DCT transform to each layer of 3D group that contains
          // patches from basic image estimate
          run_DCT2D8x8(d_gathered_stacks_basic, d_gathered_stacks_basic,
                       trans_size, num_threads_tr, num_blocks_tr);
          cuda_error_check(cudaGetLastError());
          cuda_error_check(cudaDeviceSynchronize());
          ...
          /*
          1) 1D Walsh-Hadamard transform of proper size on the 3rd dimension of
          each 3D noisy group (from noisy patches) and each 3D basic group
          (pathes from the basic image estimate) 2) Compute wiener coeficients
          from basic groups 3) Filtering: Element-wise multiplication between
          noisy group and corresponding wiener coefficinets 4) Inverse 1D
          transform to the filtered groups 5) Compute the weingt of each 3D
          group
          */
          run_wiener_filtering(
              start_point,        // IN: First reference patch of a batch
              d_gathered_stacks,  // IN/OUT: 3D groups with thransfomed noisy
                                  // patches that will be filtered
              d_gathered_stacks_basic,  // IN: 3D groups with thransfomed basic
                                        // patches estimates
              d_w_P,                    // OUT: Weight of each 3D group
              d_num_patches_in_stack,   // IN: Numbers of patches in 3D groups
              stacks_dim,      // IN: Dimensions limiting addresses of reference
          ...
          // Apply 2D IDCT transform to each layer of 3D group that contains
          // filtered patches
          run_IDCT2D8x8(d_gathered_stacks, d_gathered_stacks, trans_size,
                        num_threads_tr, num_blocks_tr);
          ...
          // Aggregate filtered patches of all 3D groups of a batch into
          // numerator and denominator buffers
          run_aggregate_block(
              start_point,        // IN: First reference patch of a batch
              d_gathered_stacks,  // IN: 3D groups with thransfomed patches
    ...
    // Divide numerator by denominator and save resullt to output image
    for (int channel = 0; channel < channels; ++channel) {
      run_aggregate_final(d_numerator[channel],     // IN: Aggregation buffer
                          d_denominator[channel],   // IN: Aggregation buffer
                          image_dim,                // IN: Image dimensions
                          denoised_image[channel],  // OUT: Image estimate
                          num_threads_f, num_blocks_f);
      cuda_error_check(cudaGetLastError());
      cuda_error_check(cudaDeviceSynchronize());
    }
    ...
    if (_verbose) {
      ...
      std::cout << "  BlockMatching took: " << time_blockmatching.getSeconds()
                << std::endl;
      std::cout << "  2x Get took: " << time_get.getSeconds() << std::endl;
      std::cout << "  2x Transform took: " << time_transform.getSeconds()
                << std::endl;
      std::cout << "  Wiener filtering took: " << time_wien.getSeconds()
                << std::endl;
      std::cout << "  Inverse transform took: " << time_itransform.getSeconds()
                << std::endl;
      std::cout << "  Aggregation took: " << time_aggregate.getSeconds()
  ...
  void copy_host_image(raw_int* dst_image, int width, int height,
                       int channels) {
    size_t image_size = width * height;
    for (int channel = 0; channel < channels; ++channel) {
      cuda_error_check(
          cudaMemcpy(dst_image + channel * image_size,  // Destination
                     d_denoised_image[channel],         // Source
                     image_size * sizeof(raw_int),      // Size
                     cudaMemcpyDeviceToHost));          // Copy direction
    }
  }

  // Free all buffers allocated on device that are dependent on 'denoising
  // parameters'.
  ...
  void free_device_image() {
    for (auto& it : d_noisy_image) cuda_error_check(cudaFree(it));
    d_noisy_image.clear();
    for (auto& it : d_denoised_image) cuda_error_check(cudaFree(it));
 ...
 public:
  BM3D()
      : h_hard_params(),
        h_wien_params(),
        d_gathered_stacks(0),
        d_gathered_stacks_basic(0),
        d_w_P(0),
        d_stacks(0),
        d_num_patches_in_stack(0),
        h_reserved_width(0),
        h_reserved_height(0),
  ...
  BM3D(uint n, uint k, uint N, uint T, uint p, float L3D, bool seceon_step)
    ...
    if (k != 8)
      throw std::invalid_argument(
          "k has to be 8, other values not implemented yet.");
  }
  ...
  src_image and dst_image are arrays allocated in the host memory and the pixels
  are stored here by the channels. First width*height pixels represent luma (Y)
  ...
  */
  void denoise_host_image(raw_int* src_image, raw_int* dst_image, int width,
    ...
    // Allocation
    if (h_reserved_width != width || h_reserved_height != height ||
        h_reserved_channels != channels || h_reserved_two_step != two_step)
      reserve(width, height, channels, two_step);

    if (h_reserved_width == 0 || h_reserved_height == 0 ||
        h_reserved_channels == 0)
      return;
  ...
  /*void denoise_device_image(raw_int *src_image, raw_int *dst_image, int width,
  int height, int channels, bool two_step)
  {
          //TODO
  }*/
