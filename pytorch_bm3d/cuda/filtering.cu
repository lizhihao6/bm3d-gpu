...
template <typename T>
__inline__ __device__ float blockReduceSum(T* shared, T val, int tid,
                                           int tcount) {
...
__device__ __forceinline__ float abspow2(float& a) { return a * a; }
...
__device__ inline void get_block_addresses(
    ...
    uint& start_idx)       // OUT: Address of a first element of the 3D group in
                           // stacks array
{
  ...
  if (outer_address.y >= stacks_dim.y &&
      outer_address.y < stacks_dim.y + params.p - 1)
    outer_address.y = stacks_dim.y - 1;
  // Ensure, that the right most patches will be taken as reference patches
  // regardless the p parameter.
  if (outer_address.x >= stacks_dim.x &&
      outer_address.x < stacks_dim.x + params.p - 1)
...
Division: One block handles one patch_stack, threads match to the pixels of a
patch
*/
__global__ void get_block(
    const uint2 start_point,          // IN: first reference patch of a batch
    const raw_int* __restrict image,  // IN: image
    const ushort* __restrict stacks,  // IN: array of adresses of similar
                                      // patches
    const uint* __restrict g_num_patches_in_stack,  // IN: numbers of patches in
                                                    // 3D groups
    ...
    const Params params)  // IN: denoising parameters
{
  uint startidx;
  uint2 outer_address;
  get_block_addresses(start_point, params.k * params.k * (params.N + 1),
                      stacks_dim, params, outer_address, startidx);

  if (outer_address.x >= stacks_dim.x || outer_address.y >= stacks_dim.y)
    return;

  patch_stack += startidx;

  const ushort* z_ptr =
      &stacks[idx3(0, blockIdx.x, blockIdx.y, params.N, gridDim.x)];

  uint num_patches =
      g_num_patches_in_stack[idx2(blockIdx.x, blockIdx.y, gridDim.x)];

  patch_stack[idx3(threadIdx.x, threadIdx.y, 0, params.k, params.k)] =
      (float)(image[idx2(outer_address.x + threadIdx.x,
                         outer_address.y + threadIdx.y, image_dim.x)]);
  for (uint i = 0; i < num_patches; ++i) {
    ...
    patch_stack[idx3(threadIdx.x, threadIdx.y, i + 1, params.k, params.k)] =
        (float)(image[idx2(outer_address.x + x + threadIdx.x,
                           outer_address.y + y + threadIdx.y, image_dim.x)]);
  }
}
...
Division: Each block delas with one transformed patch stack. (number of threads
in block should be k*k)
*/
__global__ void hard_treshold_block(
  ...
  uint startidx;
  uint2 outer_address;
  get_block_addresses(start_point, patch_stack_size, stacks_dim, params,
                      outer_address, startidx);
  ...
  uint num_patches =
      g_num_patches_in_stack[idx2(blockIdx.x, blockIdx.y, gridDim.x)] +
      1;  //+1 for the reference patch.
  float* s_patch_stack =
      data + (tid * (num_patches +
                     1));  //+1 for avoiding bank conflicts //TODO:sometimes
...
/*
Fills two buffers: numerator and denominator in order to compute weighted
average of pixels Used parameters: k,N,p Division: Each block delas with one
transformed patch stack.
*/
__global__ void aggregate_block(
    const uint2 start_point,  // IN: first reference patch of a batch
    const float* __restrict patch_stack,    // IN: 3D groups with thransfomed
                                            // patches
    const float* __restrict w_P,            // IN: weight for each 3D group
    const ushort* __restrict stacks,        // IN: array of adresses of similar
                                            // patches
    const float* __restrict kaiser_window,  // IN: kaiser window
    float* numerator,    // IN/OUT: numerator aggregation buffer (have to be
                         // initialized to 0)
    float* denominator,  // IN/OUT: denominator aggregation buffer (have to be
                         // initialized to 0)
    const uint* __restrict g_num_patches_in_stack,  // IN: numbers of patches in
                                                    // 3D groups
    const uint2 image_dim,                          // IN: image dimensions
  ...
  uint num_patches =
      g_num_patches_in_stack[idx2(blockIdx.x, blockIdx.y, gridDim.x)] + 1;
  ...
  for (uint z = 0; z < num_patches; ++z) {
    ...
    float value =
        (patch_stack[idx3(threadIdx.x, threadIdx.y, z, params.k, params.k)]);
    int idx = idx2(outer_address.x + x + threadIdx.x,
                   outer_address.y + y + threadIdx.y, image_dim.x);
...
__global__ void aggregate_final(
    ...
    const uint2 image_dim,                // IN: image dimensions
    raw_int* image_o)                     // OUT: image estimate
{
  uint idx = blockIdx.x * blockDim.x + threadIdx.x;
  uint idy = blockIdx.y * blockDim.y + threadIdx.y;
  if (idx >= image_dim.x || idy >= image_dim.y) return;

  int value = lrintf(numerator[idx2(idx, idy, image_dim.x)] /
                     denominator[idx2(idx, idy, image_dim.x)]);
  if (value < 0) value = 0;
  // if (value > 255) value = 255;
  image_o[idx2(idx, idy, image_dim.x)] = (raw_int)value;
}
...
__global__ void wiener_filtering(
    ...
    const float* __restrict patch_stack_basic,  // IN: 3D groups with
                                                // thransfomed patches of the
                                                // basic estimate
  ...
  float* s_patch_stack_basic =
      data + (tid * (num_patches + 1));  //+1 for avoiding bank conflicts
  float* s_patch_stack =
      s_patch_stack_basic +
      (tcount * (num_patches + 1));  //+1 for avoiding bank conflicts
...
extern "C" void run_get_block(const uint2 start_point,
  ...
  get_block<<<num_blocks, num_threads>>>(start_point, image, stacks,
                                         num_patches_in_stack, patch_stack,
                                         image_dim, stacks_dim, params);
}
...
extern "C" void run_aggregate_block(
  ...
  aggregate_block<<<num_blocks, num_threads>>>(
      start_point, patch_stack, w_P, stacks, kaiser_window, numerator,
      denominator, num_patches_in_stack, image_dim, stacks_dim, params);
}
...
extern "C" void run_aggregate_final(const float* __restrict numerator,
                                    ...
                                    raw_int* denoised_image,
                                    const dim3 num_threads,
                                    const dim3 num_blocks) {
  aggregate_final<<<num_blocks, num_threads>>>(numerator, denominator,
                                               image_dim, denoised_image);
}
